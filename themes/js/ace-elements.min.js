...
(function(a,b){
	a.fn.ace_spinner=function(c){
		this.each(function(){
			...
			var d=c.max||999;
			d=(""+d).length;
			var j=a(this).addClass("spinner-input").css("width",(d*10)+"px").wrap('<div class="ace-spinner">').after('<div class="spinner-buttons btn-group btn-group-vertical">						<button type="button" class="btn spinner-up btn-mini '+e+'">						<i class="'+f+'"></i>						</button>						<button type="button" class="btn spinner-down btn-mini '+g+'">						<i class="'+i+'"></i>						</button>						</div>').closest(".ace-spinner").spinner(c).wrapInner("<div class='input-append'></div>");a(this).on("mousewheel DOMMouseScroll",function(k){
...
(function(a,b){
	a.fn.ace_wizard=function(c){
		this.each(function(){
			...
			var d=h.find("li");
			var e=d.length;
			var f=parseFloat((100/e).toFixed(1))+"%";d.css({"min-width":f,"max-width":f});
			h.show().wizard();
	...
	a.fn.ace_wysiwyg=function(c,h){
		...
		this.each(function(){
			...
			for(var m in f){
				if(f.hasOwnProperty(m)){
					...
					if(typeof o=="string"&&o in g){
						...
						if(typeof o=="object"&&o.name in g){o=a.extend(g[o.name],o)}else{continue}
					}
			...
			q.find(".wysiwyg_colorpicker").each(function(){
				a(this).ace_colorpicker({pull_right:true,caret:false}).change(function(){a(this).nextAll("input").eq(0).val(this.value).change()}).next().find(".btn-colorpicker").tooltip({title:this.title,animation:false})
			});
			var k;
