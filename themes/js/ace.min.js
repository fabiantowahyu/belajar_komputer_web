jQuery(function() {
		...
		$(".nav-list").on(ace.click_event,function(g){
			...
			if(!f.hasClass("dropdown-toggle")){
				if(b&&ace.click_event=="tap"&&f.get(0).parentNode.parentNode==this){
					...
					if(g.target!=h&&!$.contains(h,g.target)){return false}
				}
				return
			}
	...
	function enable_search_ahead(){
		$("#nav-search-input").typeahead({
			...
			updater:function(a){
				$("#nav-search-input").focus();return a
			}
		})
	}
	...
	function general_things(){
		...
		$("#skin-colorpicker").ace_colorpicker().on("change",function(){
			...
			if(b=="skin-2") {
				...
				$(".ace-nav > li").removeClass("no-border margin-1");$(".ace-nav > li:not(:last-child)").removeClass("light-pink").find('> a > [class*="icon-"]').removeClass("pink").end().eq(0).find(".badge").removeClass("badge-warning")
			}
			if(b=="skin-3") {
	...
	function widget_boxes(){
		$(".page-content").delegate(".widget-toolbar > [data-action]","click",function(k){
			...
			if(a.hasClass("ui-sortable-helper")){return}
			if(l=="collapse") {
				...
				if(a.hasClass("collapsed")) {
					...
					if(i){i.addClass(b).removeClass(f)}
