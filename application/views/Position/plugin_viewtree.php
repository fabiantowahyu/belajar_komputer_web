...
<script type="text/javascript">
	function clickOrgLevel(id) {
		window.location.href = '<?php echo site_url(); ?>position/CTRL_New/'+ id;
	}
	function clickOrgLevel_Edit(id) {
		window.location.href = '<?php echo site_url(); ?>position/CTRL_Edit/'+ id;
	}
	function editOrgLevel(id) {
		PopUpWindow('<?php echo $url_view; ?>' + id,'mywindow',600,520,'yes','yes');
		return false;
	}
	...
	$(function(){
		$('#treeOrgLevel').jstree({
			'core' : {
				'data' : {
					"url" : "<?php echo site_url(); ?>position/CTRL_SelectData",
