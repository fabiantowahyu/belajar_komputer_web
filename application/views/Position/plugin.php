...
<script type="text/javascript">
	$(function() {
		...
		$('#validation-form').validate({
			...
			messages: {
				position_code: {
					remote: "Position Code already exist"
				}
			},
	...
	function ChangeValue_Grade(intWhichOne) {
		var frmShr = document.myForm;
		var arrNonMember = new Array(frmShr.selNonGrade.options.length);
		var arrIsMember = new Array(frmShr.selGrade.options.length);
		var arrMember = new Array();
		var arrNon = new Array();
		var lstNon = frmShr.selNonGrade;
		var lstMem = frmShr.selGrade;
		var intSelected = 0;
	...
	function SelectAll_Member() {
		...
		// Grade
		var lstGrade = frmMember.selGrade;
		var arrGrade = new Array();
		
		for(i=0;i<lstGrade.options.length;i++){
			arrGrade[i] = lstGrade.options[i].value;	

		}
		
		frmMember.work_location.value = arrMember.join(',');
		frmMember.grade_code.value = arrGrade.join(',');
		return true;
	}
	
	function uncheck(radioObj, newValue) {
		if(document.myForm.chkNeck.checked == false) {
			for(var i = 0; i < 2; i++) {
				radioObj[i].checked = false;
				//document.frmPos.radNeck.checked = false;  
				//alert("test");
			}
		}
	}
	
	function raduncheck(radioObj, newValue)	{
		if(document.myForm.chkNeck.checked == false) {
			for(var i = 0; i < 2; i++) {
				radioObj[i].checked = false;
				// document.frmPos.radNeck.checked = false;  
