...
class Md_information_admob extends CI_Model {

    // Fungsi Ambil Data
    public function MDL_Select() {
        $tblletter_template = $this->config->item('ttrs_information_admob');

        $hasil = array();

        $sSQL = "
			Select * from ttrs_information_admob order by recdate desc
		";
    ...
    public function MDL_SelectID($id) {
        $ttrs_information_admob = $this->config->item('ttrs_information_admob');

        return $this->db->get_where($ttrs_information_admob, array('id' => $id))->row();
    }
    ...
    public function MDL_Insert() {
        ...
        if ($this->input->post('external_source')) {
            ...
            $link = base_url() . 'information_admob/CTRL_View/' . $id;
            $external = 0;
        }
        ...
        $this->db->insert($ttrs_information_admob, $data);
    }

    public function MDL_isPermInsert($id) {
        $tblName = $this->config->item('ttrs_information_admob');
    ...
    public function MDL_Delete($id) {
        ...
        $this->db->delete($ttrs_information_admob, array('id' => $id));
    }
