...
class Information_admob extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('md_information_admob');
    }
    ...
    public function index() {
        if (!$this->auth->Auth_isPerm()) {
            ...
            $data['results'] = $this->md_information_admob->MDL_Select();
            
            //$this->auth->TVD($data['results']);die();
            //$this->auth->TVD($data['results']);die();
            $nm_title = $this->auth->Auth_getNameMenu();
            $data['title'] = sprintf("%s", $nm_title);
            $data['url_view'] = sprintf("%sinformation_admob/CTRL_View/", site_url());
            $data['page'] = 'information_admob/view';
            $data['plugin'] = 'information_admob/plugin';
            $this->load->view('template_admin', $data);
        }
    }
    ...
    public function CTRL_New() {
        if (!$this->auth->Auth_isPerm()) {
            ...
            if ($this->input->post('close')) {
                redirect('information_admob');
            } elseif ($this->input->post('submit')) {
                $isDuplicated = $this->md_information_admob->MDL_isPermInsert($this->input->post('id'));
                if ($isDuplicated) {
                    $this->md_information_admob->MDL_Insert();
                    redirect('information_admob');
                } else {
                ...
                $data['id'] = $this->md_information_admob->MDL_getAutoID();

                $nm_title = $this->auth->Auth_getNameMenu();
                $data['title_head'] = sprintf("%s - Add New", $nm_title);
                $data['title'] = sprintf("%s", $nm_title);

                $data['url'] = 'information_admob/CTRL_New';

                $data['url_browse_picture'] = sprintf("%sinformation_admob/CTRL_Browse_picture/", site_url());
                $data['page'] = 'information_admob/form';
                $data['plugin'] = 'information_admob/plugin';
                $this->load->view('template_admin', $data);
            }
        }
    ...
    public function CTRL_Edit($id = '') {
        if (!$this->auth->Auth_isPerm()) {
            ...
            if ($this->input->post('close')) {
                ...
                $this->md_information_admob->MDL_Update($id);
                redirect('information_admob');
            } else {
                /* Bread crum */
                $this->load->model('md_ref_json');
                ...
                $hasil = $this->md_information_admob->MDL_SelectID($id);
                //var_dump($hasil);die();
                $data['id'] = $hasil->id;
                ...
                $data['title'] = sprintf("%s", $nm_title);
                $data['url'] = 'information_admob/CTRL_Edit/' . $id;
                $data['url_del'] = 'information_admob/CTRL_Delete/' . $id;
                $data['page'] = 'information_admob/form_edit';
                $data['plugin'] = 'information_admob/plugin';
                $this->load->view('template_admin', $data);
            }
        }
    ...
    public function CTRL_Delete($id = '') {
        if (!$this->auth->Auth_isPerm()) {
            ...
            $this->md_information_admob->MDL_Delete($id);
            redirect('information_admob');
        }
    }
    ...
    public function CTRL_View($id,$language="") {
            ...
            $sql2 = "select page_view from ttrs_information_admob where id='$id'";
            $res2 = $this->db->query($sql2)->row();
            if($res2){
                    $data = array('page_view'=>$res2->page_view+1);
                    
                    $this->db->where('id', $id);
		$this->db->update('ttrs_information_admob', $data);
            }
            
            
            
        $content="";
        $sql = "select * from ttrs_information_admob where id ='$id'";
        ...
        $this->load->view('template_webview_information_admob', $data);
    }
    ...
    public function CTRL_Browse_picture() {
        ...
        if (!$this->auth->Auth_isPerm()) {
            ...
            if ($this->input->post()) {
                ...
                $res = $this->md_information_admob->MDL_UploadPicture();
                if ($res) {
                    $this->session->set_flashdata('upload_success', array('title' => '', 'message' => 'Success Upload Images', 'class' => 'alert alert-success'));


                    redirect('information_admob/CTRL_Browse_picture');
                }
            }

            $data['results'] = $this->md_information_admob->MDL_SelectPicture();


            $data['url'] = 'information_admob/CTRL_Browse_picture/';
            $data['page'] = 'information_admob/view_picture';
            $data['plugin'] = 'information_admob/plugin';
    ...
    public function CTRL_DeletePicture($id = '') {
        if (!$this->auth->Auth_isPerm()) {
            ...
            $this->md_information_admob->MDL_DeletePicture($id);
