...
class Information_alkes extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('md_information_alkes');
    }
    ...
    public function index() {
        if (!$this->auth->Auth_isPerm()) {
            ...
            $data['results'] = $this->md_information_alkes->MDL_Select();
            
            //$this->auth->TVD($data['results']);die();
            //$this->auth->TVD($data['results']);die();
            $nm_title = $this->auth->Auth_getNameMenu();
            $data['title'] = sprintf("%s", $nm_title);
            $data['url_view'] = sprintf("%sinformation_alkes/CTRL_View/", site_url());
            $data['page'] = 'information_alkes/view';
            $data['plugin'] = 'information_alkes/plugin';
            $this->load->view('template_admin', $data);
        }
    }
    ...
    public function CTRL_New() {
        if (!$this->auth->Auth_isPerm()) {
            ...
            if ($this->input->post('close')) {
                redirect('information_alkes');
            } elseif ($this->input->post('submit')) {
                $isDuplicated = $this->md_information_alkes->MDL_isPermInsert($this->input->post('id'));
                if ($isDuplicated) {
                    $this->md_information_alkes->MDL_Insert();
                    redirect('information_alkes');
                } else {
                ...
                $data['id'] = $this->md_information_alkes->MDL_getAutoID();

                $nm_title = $this->auth->Auth_getNameMenu();
                $data['title_head'] = sprintf("%s - Add New", $nm_title);
                $data['title'] = sprintf("%s", $nm_title);

                $data['url'] = 'information_alkes/CTRL_New';

                $data['url_browse_picture'] = sprintf("%sinformation_alkes/CTRL_Browse_picture/", site_url());
                $data['page'] = 'information_alkes/form';
                $data['plugin'] = 'information_alkes/plugin';
                $this->load->view('template_admin', $data);
            }
        }
    ...
    public function CTRL_Edit($id = '') {
        if (!$this->auth->Auth_isPerm()) {
            ...
            if ($this->input->post('close')) {
                ...
                $this->md_information_alkes->MDL_Update($id);
                redirect('information_alkes');
            } else {
                /* Bread crum */
                $this->load->model('md_ref_json');
                ...
                $hasil = $this->md_information_alkes->MDL_SelectID($id);
                //var_dump($hasil);die();
                $data['id'] = $hasil->id;
                ...
                $data['title'] = sprintf("%s", $nm_title);
                $data['url'] = 'information_alkes/CTRL_Edit/' . $id;
                $data['url_del'] = 'information_alkes/CTRL_Delete/' . $id;
                $data['page'] = 'information_alkes/form_edit';
                $data['plugin'] = 'information_alkes/plugin';
                $this->load->view('template_admin', $data);
            }
        }
    ...
    public function CTRL_Delete($id = '') {
        if (!$this->auth->Auth_isPerm()) {
            ...
            $this->md_information_alkes->MDL_Delete($id);
            redirect('information_alkes');
        }
    }
    ...
    public function CTRL_View($id,$language="") {
        ...
        $content="";
        $sql = "select * from ttrs_information_alkes where id ='$id'";
        ...
        $this->load->view('template_webview_information_alkes', $data);
    }
    ...
    public function CTRL_Browse_picture() {
        ...
        if (!$this->auth->Auth_isPerm()) {
            ...
            if ($this->input->post()) {
                ...
                $res = $this->md_information_alkes->MDL_UploadPicture();
                if ($res) {
                    $this->session->set_flashdata('upload_success', array('title' => '', 'message' => 'Success Upload Images', 'class' => 'alert alert-success'));


                    redirect('information_alkes/CTRL_Browse_picture');
                }
            }

            $data['results'] = $this->md_information_alkes->MDL_SelectPicture();


            $data['url'] = 'information_alkes/CTRL_Browse_picture/';
            $data['page'] = 'information_alkes/view_picture';
            $data['plugin'] = 'information_alkes/plugin';
    ...
    public function CTRL_DeletePicture($id = '') {
        if (!$this->auth->Auth_isPerm()) {
            ...
            $this->md_information_alkes->MDL_DeletePicture($id);
